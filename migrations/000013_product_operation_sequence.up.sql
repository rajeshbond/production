-- =========================================
-- PRE-REQUISITE (Dependency-Safe Version)
-- =========================================

-- We use an anonymous DO block to check existence without dropping
-- This prevents the "cannot drop because other objects depend on it" error.

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uix_product_id_tenant') THEN
        ALTER TABLE product ADD CONSTRAINT uix_product_id_tenant UNIQUE (id, tenant_id);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uix_operation_id_tenant') THEN
        ALTER TABLE operation_master ADD CONSTRAINT uix_operation_id_tenant UNIQUE (id, tenant_id);
    END IF;
END $$;
...
-- =========================================
-- It is safe to drop this one because it's on the NEW table
...
-- Final check for the redundant naming (uix_product_tenant)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'uix_product_tenant') THEN
        ALTER TABLE product ADD CONSTRAINT uix_product_tenant UNIQUE (id, tenant_id);
    END IF;
END $$;